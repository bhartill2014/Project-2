...
* * you may use ECE2552::allocation_table.summary();
* *
* * You must simply indicate that you want to start recording by
* * using ECE2552::alocation_table.summary();
* *
* **************************************************************************
****************************************************************************/
...
*
*   1. The first namespace ECE2552 is associated with tools used by the
*      second part.
...
void *operator new(size_t size) throw(std::bad_alloc){
	...
	ECE2552::allocation_table.insert(ptr, size, false);
...
void operator delete(void *ptr) throw () {
	ECE2552::allocation_table.remove(ptr, false);
	free(ptr);
}
...
void *operator new[](size_t size) throw(std::bad_alloc) {
	char *ptr = static_cast<char *>(malloc(size + 2 * ECE2552::PAD));
	ECE2552::allocation_table.insert(static_cast<void *>(ptr + ECE2552::PAD), size, true);
	ECE2552::initialize_array_bounds(ptr, size + 2 * ECE2552::PAD);
	return static_cast<void *>(ptr + ECE2552::PAD);
}
...
void operator delete[](void *ptr) throw () {
	size_t size = ECE2552::allocation_table.remove(ptr, true);

	if (ECE2552::allocation_table.is_recording()) {
		ECE2552::check_array_bounds(static_cast<char *>(ptr)-ECE2552::PAD, size + 2 * ECE2552::PAD);
	}

	free(static_cast<char *>(ptr)-ECE2552::PAD);
}
